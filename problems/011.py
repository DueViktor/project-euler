...
def horizontal_right(matrix):
    max = 0
    for row in range(len(matrix)):
        for col in range(len(matrix)):
            if len(matrix) >= col + 4:
                val = matrix[row][col] * matrix[row][col+1] * matrix[row][col+2] * matrix[row][col+3]
                if val > max: max = val
    return max

def vertical_down(matrix):
    matrix = matrix.T
    return horizontal_right(matrix)
    ...
    for row in range(len(matrix)):
        for col in range(len(matrix)):
            if len(matrix) >= col + 4 and len(matrix) >= row + 4:
                val = matrix[row][col] * matrix[row+1][col+1] * matrix[row+2][col+2] * matrix[row+3][col+3]
                if val > max: max = val
    return max
...
def test(matrix):
    max_vals = [
                horizontal_right(matrix),
                diagonal_left(matrix),
                vertical_down(matrix),
                diagonal_right(matrix)
                ]
    return max(max_vals)

print("The maximum product of four numbers are: {}".format(test(data)))
